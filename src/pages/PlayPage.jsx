...
export default function PlayPage() {
  ...
  async function fetchAll() {
    await Promise.all([fetchRound(), fetchPlayersAndScores()])
    setLoading(false)
  }
  ...
  async function saveScore() {
    ...
    setSaveMsg('')
    const existing = scores.find(s => s.round_player_id === myPlayer.id && s.hole?.hole_number === selectedHole)
    const payload = { round_player_id: myPlayer.id, hole_id: hole.id, gross_score: grossScore }
    let error
    if (existing) {
  ...
  function getScoreStyle(gross, par) {
    ...
    if (diff <= -2) return { color: '#1D9E75', fontWeight: 600 }
    if (diff === -1) return { color: '#dc2626', fontWeight: 600 }
    if (diff === 0)  return { color: 'var(--gray-900)', fontWeight: 500 }
    if (diff === 1)  return { color: '#185FA5', fontWeight: 500 }
    return { color: 'var(--gray-900)', fontWeight: 700 }
  }

  const myPlayer = players.find(p => p.profile_id === user.id)
  const currentHole = holes.find(h => h.hole_number === selectedHole)
  const leaderboard = getLeaderboard()
  const myStats = myPlayer ? getPlayerStats(myPlayer) : null

  if (loading) return (
    <div className="page" style={{ paddingTop: 80, textAlign: 'center', color: 'var(--gray-500)' }}>Loading round…</div>
  )
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', paddingBottom: 40 }}>
      ...
      <div style={{ background: 'var(--green-700)', padding: '16px 16px 0', color: 'white' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
            ...
            {isOrganiser && !showEndConfirm && (
              <button onClick={() => setShowEndConfirm(true)} style={{ fontSize: 11, background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', padding: '3px 10px', borderRadius: 12, cursor: 'pointer' }}>
                End round
              </button>
            )}
            {isOrganiser && showEndConfirm && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => setShowEndConfirm(false)} style={{ fontSize: 11, background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', padding: '3px 10px', borderRadius: 12, cursor: 'pointer' }}>Cancel</button>
                <button onClick={endRound} disabled={ending} style={{ fontSize: 11, background: 'rgba(220,38,38,0.8)', border: 'none', color: 'white', padding: '3px 10px', borderRadius: 12, cursor: 'pointer', fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '8px 0', fontSize: 13, fontWeight: 500, background: 'none', border: 'none', color: tab === t ? 'white' : 'rgba(255,255,255,0.6)', borderBottom: tab === t ? '2px solid white' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tab === 'Leaderboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {leaderboard.map((player, rank) => {
              ...
              return (
                <div key={player.id} className="card" style={{ padding: '12px 16px', border: player.profile_id === user.id ? '1.5px solid var(--green-500)' : undefined }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ fontSize: 18, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--gray-300)', width: 24 }}>{rank + 1}</div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 12, color: 'var(--gray-500)' }}>{stats.holesPlayed} hole{stats.holesPlayed !== 1 ? 's' : ''} · hcp {player.playing_handicap}</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'flex-end' }}>
                        <span style={{ fontSize: 12, color: 'var(--gray-700)' }}><span style={{ fontWeight: 700 }}>F9</span> {stats.front9Points}</span>
                        <span style={{ fontSize: 12, color: 'var(--gray-700)' }}><span style={{ fontWeight: 700 }}>B9</span> {stats.holesPlayed > 9 ? stats.back9Points : '–'}</span>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 24, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--green-700)', lineHeight: 1 }}>{stats.totalPoints}</p>
                  ...
                  {stats.holesPlayed > 0 && (
                    <div style={{ display: 'flex', gap: 3, marginTop: 10, flexWrap: 'wrap' }}>
                      {holes.map(hole => {
                        ...
                        return (
                          <div key={hole.hole_number} title={`Hole ${hole.hole_number}: ${s ? `${s.gross_score} (${pts} pts)` : 'not played'}`}
                            style={{ width: 22, height: 22, borderRadius: 4, background: pts === null ? 'var(--gray-100)' : pts >= 3 ? '#1D9E75' : pts === 2 ? 'var(--green-100)' : pts === 1 ? 'var(--amber-100)' : 'var(--red-100)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 600, color: pts === null ? 'var(--gray-300)' : pts >= 3 ? 'white' : pts === 2 ? 'var(--green-700)' : pts === 1 ? 'var(--amber-500)' : 'var(--red-500)' }}>
                            {pts !== null ? pts : '·'}
                          </div>
                        )
                      })}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  ...
                  <tr style={{ background: 'var(--green-800, #0f2419)' }}>
                    <th style={{ padding: '4px 8px' }} /><th style={{ padding: '4px 6px' }} /><th style={{ padding: '4px 6px' }} />
                    {players.map(p => (
                      <>
                ...
                <tbody>
                  ...
                  <tr style={{ background: 'var(--gray-100)' }}><td colSpan={3 + players.length * 2} style={{ padding: '2px 0' }} /></tr>
                  <tr style={{ background: 'var(--gray-50)' }}>
                    <td colSpan={2} style={{ padding: '8px 8px', fontWeight: 600, fontSize: 12, color: 'var(--gray-600)' }}>Course par</td>
                    <td style={{ padding: '8px 6px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--gray-700)' }}>{holes.reduce((sum, h) => sum + h.par, 0)}</td>
                    {players.map(p => <><td key={p.id + '-par'} colSpan={2} style={{ padding: '8px 4px', textAlign: 'center', color: 'var(--gray-400)', fontSize: 12, borderLeft: '1px solid var(--gray-100)' }}>—</td></>)}
                  </tr>
                  <tr style={{ background: 'var(--gray-50)' }}>
                    <td colSpan={3} style={{ padding: '8px 8px', fontWeight: 600, fontSize: 12, color: 'var(--gray-600)' }}>Total gross</td>
                    {players.map(p => {
                      const totalGross = scores.filter(s => s.round_player_id === p.id).reduce((sum, s) => sum + s.gross_score, 0)
                      return (
                        <>
                          <td key={p.id + '-tg'} style={{ padding: '8px 4px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--gray-700)', borderLeft: '1px solid var(--gray-100)' }}>{totalGross || '–'}</td>
                  ...
                  <tr style={{ background: 'var(--green-50)', borderTop: '1px solid var(--green-200)' }}>
                    ...
                    {players.map(p => (
                      <>
                        ...
                        <td key={p.id + '-tp2'} style={{ padding: '10px 4px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--green-700)', fontSize: 15 }}>{getPlayerStats(p).totalPoints}</td>
                      </>
                    ))}
                  </tr>
                </tbody>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
              {holes.map(hole => {
                ...
                return (
                  <button key={hole.id} onClick={() => setSelectedHole(hole.hole_number)}
                    style={{ width: 36, height: 36, borderRadius: 8, border: isSelected ? '2px solid var(--green-600)' : '1px solid var(--gray-300)', background: isSelected ? 'var(--green-600)' : s ? 'var(--green-50)' : 'white', color: isSelected ? 'white' : s ? 'var(--green-700)' : 'var(--gray-700)', fontSize: 13, fontWeight: isSelected || s ? 600 : 400, cursor: 'pointer' }}>
                    {hole.hole_number}
                  </button>
                )
              })}
            ...
            {currentHole && (
              <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
                ...
                <div className="card" style={{ flex: 1, textAlign: 'center', padding: '10px' }}>
                  ...
                  <p style={{ fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--green-600)' }}>+{strokesReceived(myPlayer.playing_handicap, currentHole.stroke_index)}</p>
                </div>
              </div>
            )}

            <div className="card" style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 13, color: 'var(--gray-500)', marginBottom: 12, textAlign: 'center' }}>Hole {selectedHole} — your gross score</p>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 24, marginBottom: 8 }}>
                <button onClick={() => setGrossScore(s => Math.max(1, s - 1))} style={{ width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--gray-300)', background: 'white', fontSize: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: 56, fontWeight: 700, fontFamily: 'var(--font-mono)', lineHeight: 1, ...(currentHole ? getScoreStyle(grossScore, currentHole.par) : { color: 'var(--gray-900)' }) }}>{grossScore}</p>
                </div>
                <button onClick={() => setGrossScore(s => Math.min(20, s + 1))} style={{ width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--gray-300)', background: 'white', fontSize: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
              </div>
              {currentHole && <p style={{ textAlign: 'center', fontSize: 14, color: 'var(--gray-500)', marginBottom: 4 }}>{scoreLabel(grossScore, currentHole.par)}</p>}
            ...
            <button className="btn btn-primary" onClick={saveScore} disabled={saving} style={{ width: '100%', marginBottom: 8 }}>
              {saving ? 'Saving…' : `Save score for hole ${selectedHole}`}
            </button>
            {saveMsg && <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--green-600)', fontWeight: 500 }}>{saveMsg}</p>}
          </div>
        )}
      </div>
    </div>
